...
alias lt='ls --tree --depth=2' # ls tree: creates tree structure only going 2 up in structure
alias lsa='ls -A' # ls All: all hidden minus . && ..
alias ll='lsa -lh --git --header' # long ls: metadata, human-readable, git status, table headers
alias llt='lsa --tree --depth=2' # long lt: tree with hidden minus . && ..
...
function set_prompt_color_vi_mode {
    if [[ $KEYMAP == vicmd ]]; then
        PROMPT='%F{blue}[%F{yellow}%~%F{blue}] ${vcs_info_msg_0_}
%F{yellow}V%{$reset_color%}%f '
    else
        PROMPT='%F{blue}[%F{green}%~%F{blue}] ${vcs_info_msg_0_}
